...
Bullet::Bullet(XMVECTOR position, XMVECTOR direction, float projectileSpeed) : currentLifeTime(clock()), maxLifeTime(2.5f)
{
	
	SetPosition(position);
	XMVECTOR normalisedDirection = XMVector3Normalize(direction);
	XMStoreFloat3(&velocity_, normalisedDirection * projectileSpeed);
}
